(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))i(t);new MutationObserver(t=>{for(const n of t)if(n.type==="childList")for(const u of n.addedNodes)u.tagName==="LINK"&&u.rel==="modulepreload"&&i(u)}).observe(document,{childList:!0,subtree:!0});function o(t){const n={};return t.integrity&&(n.integrity=t.integrity),t.referrerPolicy&&(n.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?n.credentials="include":t.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function i(t){if(t.ep)return;t.ep=!0;const n=o(t);fetch(t.href,n)}})();const b="https://jews-saturday-compensation-blogs.trycloudflare.com",a={tab:"dashboard",status:null,menus:[],features:[],loading:!1,message:"",menuForm:{command:"",description:"",text:"",logo:""}};async function s(e,r={}){const o=await fetch(b+e,{...r,headers:{"Content-Type":"application/json",...r.headers||{}}}),i=await o.text();let t;try{t=JSON.parse(i)}catch{throw new Error(i||"Response error")}if(!o.ok)throw new Error(t.error||"API Error");return t}async function p(){try{a.status=await s("/api/status")}catch(e){a.message=e.message}}async function m(){try{a.menus=await s("/api/menus")}catch{a.menus=[]}}async function d(){try{a.features=await s("/api/features")}catch{a.features=[]}}async function f(){const e=document.querySelector("#featureName").value,r=document.querySelector("#featureCommand").value,o=document.querySelector("#featureDescription").value;if(!e||!r){alert("Nama dan command wajib diisi");return}try{await s("/api/features",{method:"POST",body:JSON.stringify({name:e,command:r,description:o})}),await d(),c()}catch(i){alert(i.message)}}async function h(e,r){try{await s("/api/features/"+e,{method:"PUT",body:JSON.stringify({enabled:r})}),await d(),c()}catch(o){alert(o.message)}}async function y(e){if(confirm("Hapus feature ini?"))try{await s("/api/features/"+e,{method:"DELETE"}),await d(),c()}catch(r){alert(r.message)}}async function l(e){await s("/api/mode",{method:"POST",body:JSON.stringify({mode:e})}),await p(),c()}function c(){const e=a.status?.connected;document.querySelector("#app").innerHTML=`
